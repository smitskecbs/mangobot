...
function publicStatusForSession(record) {
...
async function confirmDelivery(rawToken, signature, options = {}) {
  ...
  if (store.usedSignatures[signature]) {
    ...
    if (previous === session.record.deliveryId) {
      return { ok: true, idempotent: true, signature };
    }
  ...
  const done = completeRewardSent(session.record.rewardId, signature, now, options.rewardsFile);
  if (!done.ok) {
    return { ok: false, reason: done.reason, error: publicError(done.reason) };
  }
  return { ok: true, signature, kind: "reward", idempotent: Boolean(done.idempotent) };
}
